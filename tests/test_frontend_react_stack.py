...
class TestFrontendReactStack(unittest.TestCase):
    ...
    def test_trades_page_contains_trading_workspace_blocks(self):
        ...
        for token in ['FAST BUY', 'BUY+LIST', 'PnL PRO', 'Positions', 'Holdings', 'History', 'Wallet activity', 'AutoSell rules', 'LIST', 'CANCEL', 'SELL', 'TRANSFER', 'Повторить выставление', 'optimisticHistory', 'subscribeTradesStream', 'subscribePnlStream', 'seenSseKeysRef', 'scheduleSseRefresh', 'refreshWorkspace', 'stableJson', 'decision_trace', 'reasons:', 'risk_flags:', 'sendTransaction', 'payload_hash', 'Holding не найден для выбранного варианта.', 'Подключите TON wallet перед отправкой транзакции.', 'expandedPositionId', 'expandedHoldingId', 'listing_meta:', 'transfer_meta:', 'Коллекция', 'Модель', 'Фон', 'Узор', 'variant_id будет выбран автоматически', 'resolveVariantByTraits', 'getCollections', 'getVariants', 'GmzSelect']:
            self.assertIn(token, page)
