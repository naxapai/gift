...
class TestListingsV1(unittest.TestCase):
    ...
    def test_listing_source_status_contract(self) -> None:
        ...
        self.assertIn("strict_primary", payload)
    ...
    def test_listings_signals_resolves_variant_from_alternative_attr_keys(self) -> None:
        svc = GiftAnalyticsService()
        variant_id = "moneypots|porridge|grape|paper_crane"
        svc.bases = {
            "moneypots": SimpleNamespace(name="Money Pots", preview_url="https://example.com/money-pots.png"),
        ...
        expected = svc._v1_signal(svc.variants[variant_id], mode="tz")
        mocked_events = {
            "items": [
                {
                    "topic": "market.listing.new",
                    "ts": "2026-03-12T01:00:00Z",
                    "source": "mtproto_api",
                    "collection_id": "moneypot",
                    "collection_name": "Money Pots",
                    "listing_key": "moneypot:200",
                    "variant_id": "moneypot|unknown|unknown|unknown",
                    "preview_url": "",
                    "resell_currency": "TON",
                    "resell_amount": 1.0,
                    "attributes": {
                        "model_name": "Porridge",
                        "background_name": "Grape",
                        "pattern_name": "Paper Crane",
                    },
                }
            ],
            "source": "mtproto_api",
        ...
        row = rows[0]
        self.assertEqual(str(row.get("variant_id") or ""), variant_id)
        self.assertEqual(str(row.get("action") or ""), str(expected.get("action") or ""))
        self.assertEqual(float(row.get("score100") or 0.0), float(expected.get("score100") or 0.0))
        self.assertEqual(float(row.get("conf_pct") or 0.0), float(expected.get("conf_pct") or 0.0))
